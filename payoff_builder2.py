...
def build_library(S0, S):
    lib = []
    # On crée une grille de strikes dynamique autour de S0 (de 50% à 150% par pas de 2.5%)
    # Un pas plus fin permet une meilleure détection des dessins manuels
    grid_pct = np.arange(0.50, 1.55, 0.025)
    strikes = S0 * grid_pct

    # --- STRUCTURES À 1 STRIKE ---
    for K in strikes:
        # Straddles
        lib.append({'name': 'Straddle', 'detail': f'Long call + Long put K={K:.0f}',
            'payoff': np.maximum(S-K,0) + np.maximum(K-S,0),
            'legs': [{'type':'call','strike':K,'weight':+1}, {'type':'put', 'strike':K,'weight':+1}]})
        lib.append({'name': 'Short Straddle', 'detail': f'Short call + Short put K={K:.0f}',
            'payoff': -np.maximum(S-K,0) - np.maximum(K-S,0),
            'legs': [{'type':'call','strike':K,'weight':-1}, {'type':'put', 'strike':K,'weight':-1}]})
        ...
        lib.append({'name':'Covered Call','detail':f'Long stock / Short call K={K:.0f}',
            'payoff': S - np.maximum(S-K,0), 'legs':[{'type':'call','strike':K,'weight':-1}]})
        lib.append({'name':'Protective Put','detail':f'Long stock + Long put K={K:.0f}',
            'payoff': S + np.maximum(K-S,0), 'legs':[{'type':'put','strike':K,'weight':+1}]})

    # --- STRUCTURES À 2 STRIKES (Combinaisons K1 < K2) ---
    for K1, K2 in itertools.combinations(strikes, 2):
        # Spreads
        lib.append({'name': 'Bull Call Spread', 'detail': f'Long call K={K1:.0f} / Short call K={K2:.0f}',
            'payoff': np.maximum(S-K1,0) - np.maximum(S-K2,0),
            'legs': [{'type':'call','strike':K1,'weight':+1}, {'type':'call','strike':K2,'weight':-1}]})
        lib.append({'name': 'Bear Put Spread', 'detail': f'Long put K={K2:.0f} / Short put K={K1:.0f}',
            'payoff': np.maximum(K2-S,0) - np.maximum(K1-S,0),
            'legs': [{'type':'put','strike':K2,'weight':+1}, {'type':'put','strike':K1,'weight':-1}]})
        lib.append({'name': 'Bull Put Spread', 'detail': f'Short put K={K2:.0f} / Long put K={K1:.0f}',
            'payoff': -np.maximum(K2-S,0) + np.maximum(K1-S,0),
            'legs': [{'type':'put','strike':K2,'weight':-1}, {'type':'put','strike':K1,'weight':+1}]})
        lib.append({'name': 'Bear Call Spread', 'detail': f'Short call K={K1:.0f} / Long call K={K2:.0f}',
            'payoff': -np.maximum(S-K1,0) + np.maximum(S-K2,0),
            'legs': [{'type':'call','strike':K1,'weight':-1}, {'type':'call','strike':K2,'weight':+1}]})
        
        # Strangles & co
        lib.append({'name': 'Strangle', 'detail': f'Long call K={K2:.0f} + Long put K={K1:.0f}',
            'payoff': np.maximum(S-K2,0) + np.maximum(K1-S,0),
            'legs': [{'type':'call','strike':K2,'weight':+1}, {'type':'put', 'strike':K1,'weight':+1}]})
        lib.append({'name': 'Short Strangle', 'detail': f'Short call K={K2:.0f} + Short put K={K1:.0f}',
            'payoff': -np.maximum(S-K2,0) - np.maximum(K1-S,0),
            'legs': [{'type':'call','strike':K2,'weight':-1}, {'type':'put', 'strike':K1,'weight':-1}]})
        lib.append({'name': 'Strip', 'detail': f'Long call K={K2:.0f} + 2x Long put K={K1:.0f}',
            'payoff': np.maximum(S-K2,0) + 2*np.maximum(K1-S,0),
            'legs': [{'type':'call','strike':K2,'weight':+1}, {'type':'put', 'strike':K1,'weight':+2}]})
        lib.append({'name': 'Strap', 'detail': f'2x Long call K={K2:.0f} + Long put K={K1:.0f}',
            'payoff': 2*np.maximum(S-K2,0) + np.maximum(K1-S,0),
            'legs': [{'type':'call','strike':K2,'weight':+2}, {'type':'put', 'strike':K1,'weight':+1}]})
        lib.append({'name': 'Risk Reversal', 'detail': f'Long call K={K2:.0f} / Short put K={K1:.0f}',
            'payoff': np.maximum(S-K2,0) - np.maximum(K1-S,0),
            'legs': [{'type':'call','strike':K2,'weight':+1}, {'type':'put', 'strike':K1,'weight':-1}]})
        lib.append({'name': 'Collar', 'detail': f'Long put K={K1:.0f} / Short call K={K2:.0f}',
            'payoff': np.maximum(K1-S,0) - np.maximum(S-K2,0),
            'legs': [{'type':'put', 'strike':K1,'weight':+1}, {'type':'call','strike':K2,'weight':-1}]})
        
        # Ratios
        lib.append({'name': 'Call Spread Ratio 1x2', 'detail': f'Long call K={K1:.0f} / Short 2x call K={K2:.0f}',
            'payoff': np.maximum(S-K1,0) - 2*np.maximum(S-K2,0),
            'legs': [{'type':'call','strike':K1,'weight':+1}, {'type':'call','strike':K2,'weight':-2}]})
        lib.append({'name': 'Put Spread Ratio 1x2', 'detail': f'Long put K={K2:.0f} / Short 2x put K={K1:.0f}',
            'payoff': np.maximum(K2-S,0) - 2*np.maximum(K1-S,0),
            'legs': [{'type':'put','strike':K2,'weight':+1}, {'type':'put','strike':K1,'weight':-2}]})

    # --- STRUCTURES À 3 STRIKES (Symmetrical Butterflies & Asymmetrical Ladders) ---
    # Pour les Butterflies, on force la symétrie autour du centre Km
    for Km in strikes:
        for width in [S0*w for w in [0.05, 0.075, 0.10, 0.15, 0.20]]:
            Kl, Kh = Km - width, Km + width
            if Kl >= strikes[0] and Kh <= strikes[-1]:
                pf = np.maximum(S-Kl,0) - 2*np.maximum(S-Km,0) + np.maximum(S-Kh,0)
                lib.append({'name': 'Butterfly', 'detail': f'Long K={Kl:.0f} / Short 2x K={Km:.0f} / Long K={Kh:.0f}',
                    'payoff': pf,
                    'legs': [{'type':'call','strike':Kl,'weight':+1}, {'type':'call','strike':Km,'weight':-2}, {'type':'call','strike':Kh,'weight':+1}]})
                lib.append({'name': 'Short Butterfly', 'detail': f'Short K={Kl:.0f} / Long 2x K={Km:.0f} / Short K={Kh:.0f}',
                    'payoff': -pf,
                    'legs': [{'type':'call','strike':Kl,'weight':-1}, {'type':'call','strike':Km,'weight':+2}, {'type':'call','strike':Kh,'weight':-1}]})
                
                pf_ib = np.maximum(Kl-S,0) - np.maximum(Km-S,0) - np.maximum(S-Km,0) + np.maximum(S-Kh,0)
                lib.append({'name': 'Iron Butterfly', 'detail': f'Long put K={Kl:.0f} / Short ATM / Long call K={Kh:.0f}',
                    'payoff': pf_ib,
                    'legs': [{'type':'put', 'strike':Kl,'weight':+1}, {'type':'put', 'strike':Km,'weight':-1},
                             {'type':'call','strike':Km,'weight':-1}, {'type':'call','strike':Kh,'weight':+1}]})

    # Call Ladders (toutes les combinaisons K1 < K2 < K3)
    # Pour ne pas exploser la mémoire, on limite les ladders aux pas de 5% (en sautant un point de la grille sur deux)
    strikes_ladder = strikes[::2] 
    for K1, K2, K3 in itertools.combinations(strikes_ladder, 3):
        lib.append({'name': 'Call Ladder', 'detail': f'Long K={K1:.0f} / Short K={K2:.0f} / Short K={K3:.0f}',
            'payoff': np.maximum(S-K1,0) - np.maximum(S-K2,0) - np.maximum(S-K3,0),
            'legs': [{'type':'call','strike':K1,'weight':+1}, {'type':'call','strike':K2,'weight':-1}, {'type':'call','strike':K3,'weight':-1}]})

    # --- STRUCTURES À 4 STRIKES (Condors) ---
    # Idem, on utilise une grille allégée pour les 4 pattes pour garder l'app fluide
    strikes_condor = strikes[::2]
    for K1, K2, K3, K4 in itertools.combinations(strikes_condor, 4):
        pfc = (np.maximum(S-K1,0) - np.maximum(S-K2,0) - np.maximum(S-K3,0) + np.maximum(S-K4,0))
        lib.append({'name': 'Condor', 'detail': f'Long K={K1:.0f} / Short K={K2:.0f} / Short K={K3:.0f} / Long K={K4:.0f}',
            'payoff': pfc,
            'legs': [{'type':'call','strike':K1,'weight':+1}, {'type':'call','strike':K2,'weight':-1},
                     {'type':'call','strike':K3,'weight':-1}, {'type':'call','strike':K4,'weight':+1}]})
        
        pfic = (np.maximum(K1-S,0) - np.maximum(K2-S,0) - np.maximum(S-K3,0) + np.maximum(S-K4,0))
        lib.append({'name': 'Iron Condor', 'detail': f'Long put K={K1:.0f} / Short put K={K2:.0f} / Short call K={K3:.0f} / Long call K={K4:.0f}',
            'payoff': pfic,
            'legs': [{'type':'put', 'strike':K1,'weight':+1}, {'type':'put', 'strike':K2,'weight':-1},
                     {'type':'call','strike':K3,'weight':-1}, {'type':'call','strike':K4,'weight':+1}]})

    # --- STRUCTURES DIVERSES (Produits structurés) ---
    # Celles-ci restent bien définies via des pourcentages fixes
    for part in [0.3, 0.5, 0.7, 0.8, 1.0, 1.2]:
        for prot in [0.80, 0.85, 0.90, 0.95, 1.00]:
            lib.append({'name': 'Capital protégé', 'detail': f'Protection {prot*100:.0f}% + participation {part*100:.0f}%',
                'payoff': prot*S0 + part*np.maximum(S-S0,0), 'legs': [{'type':'call','strike':S0,'weight':part}]})

    for K in [S0*0.80, S0*0.85, S0*0.90, S0*0.95]:
        coupon = (S0 - K) / S0
        lib.append({'name': 'Reverse Convertible', 'detail': f'Coupon {coupon*100:.0f}% / Barrière K={K:.0f}',
            'payoff': S0*(1+coupon) - np.maximum(K-S,0), 'legs': [{'type':'put','strike':K,'weight':-1}]})
...
<div style='font-family:Roboto Mono,monospace;font-size:11px;color:#666;
    ...
    L'app identifie la structure standard la plus proche parmi des milliers de combinaisons.
</div>
""", unsafe_allow_html=True)
...
if drawn_payoff is not None:
    ...
    with tab1:
        if decompose_btn:
            with st.spinner("Analyse des milliers de combinaisons en cours..."):
